...
namespace m0st4fa {
	...
	ParserResult LRParser<GrammarT, LexicalAnalyzerT, SymbolT, ParsingTableT, FSMTableT, InputT>::parse(ErrorRecoveryType errorRecoveryType)
	{
		...
		* Loop inifinitly until: the input is finished, the parser accepts or an error is produced.
		...
		* _reduce(J):
			* Get to production P indexed J and do the following:
				* If the last element of P is an
		* 
		*/
		...
		while (true) {
			...
			if (this->currEntry.type == LRTableEntryType::TET_ERROR) {
